...
public class AnganwadiChildrenServiceImpl implements AnganwadiChildrenService {
	...
	@Autowired
	public AnganwadiChildrenServiceImpl(AnganwadiChildrenRepository anganwadiChildrenRepository,
			FileManagementService fileManagementService, AttendanceRepository attendanceRepository,
			ModelMapper modelMapper, FamilyRepository familyRepository, FamilyMemberRepository familyMemberRepository,
			...
			AnganwadiActivitiesRepository anganwadiActivitiesRepository, MealsTypeRepository mealsTypeRepository) {
		...
		if (findExistingRecord.size() > 0) {
			for (AnganwadiChildren record : findExistingRecord) {
				...
				saveAdmission = AnganwadiChildren.builder()
						...
						.isRegistered(saveAdmissionDTO.isRegistered()).centerId(centerId).isGoingSchool("0")
						.mobileNumber(
								saveAdmissionDTO.getMobileNumber() == null ? "" : saveAdmissionDTO.getMobileNumber())
	...
	public SaveAdmissionDTO updateRegisteredValue(String id, boolean isRegistered) {
		...
		return SaveAdmissionDTO.builder().id(findId.getId() == null ? "" : findId.getId())
				...
				.isRegistered(findId.isRegistered()).centerName(memberDetails.getCenterName())
				...
				.profilePic(findId.getProfilePic() == null ? "" : findId.getProfilePic()).build();
	}

	@Override
	public List<AttendanceDTO> makeAttendanceManual(List<AttendanceDTO> attendanceDTO, String centerId)
			throws ParseException {
		List<AttendanceDTO> addInList = new ArrayList<>();

		for (AttendanceDTO list : attendanceDTO) {
			List<AnganwadiChildren> findChildren = anganwadiChildrenRepository
					.findAllByChildIdAndRegisteredTrue(list.getChildId());
			...
			if (findAttRecords.size() > 0) {
				...
				for (Attendance updateStatus : findAttRecords) {
					...
					AttendanceDTO singleEntry = AttendanceDTO.builder()
							...
							.dob(commonMethodsService.dateChangeToString(memberDetails.getDob()) == null ? ""
									: commonMethodsService.dateChangeToString(memberDetails.getDob()))
							.name(memberDetails.getName() == null ? "" : memberDetails.getName())

							.dob(commonMethodsService.dateChangeToString(
									commonMethodsService.findMember(updateStatus.getChildId()).getDob()))
							...
							.date(timestamp).attType(list.getAttType() == null ? "" : list.getAttType())
							.att(list.getAtt() == null ? "" : list.getAtt())
							.attendance(list.getAtt() == null ? "" : list.getAtt()).build();
			...
			if (findAttRecords.size() <= 0) {
				for (AnganwadiChildren sc : findChildren) {
					...
					Attendance saveStatus = Attendance.builder()
							// .centerName(memberDetails.getCenterName() == null ? "" :
							// memberDetails.getCenterName())
							.centerId(centerId == null ? "" : centerId)
							.childId(sc.getChildId() == null ? "" : sc.getChildId())

							// .dob(dateChangeToString(memberDetails.getDob()) == null ? "" :
							// dateChangeToString(memberDetails.getDob()))
							...
							// .gender(memberDetails.getGender() == null ? "" : memberDetails.getGender())
							.isRegistered(sc.isRegistered()).date(timestamp)
							.attType(list.getAttType() == null ? "" : list.getAttType())
							.attendance(list.getAtt() == null ? "" : list.getAtt())
							.attType(list.getAttType() == null ? "" : list.getAttType()).build();
					attendanceRepository.save(saveStatus);

					AttendanceDTO singleEntry = AttendanceDTO.builder().centerName(memberDetails.getCenterName())
							.childId(saveStatus.getChildId())
							.dob(commonMethodsService.dateChangeToString(memberDetails.getDob()))
							// .dob(saveStatus.getDob())
							.name(memberDetails.getName()).latitude(saveStatus.getLatitude())
							.longitude(saveStatus.getLongitude()).photo(saveStatus.getPhoto())
							.gender(memberDetails.getGender()).date(timestamp).attType(list.getAttType())
							.att(list.getAtt()).attendance(list.getAtt()).build();
					addInList.add(singleEntry);
				}
			}
		}
	...
	public List<FamilyMemberConverted> convertUnixToDate() {
		...
		for (FamilyMember fm : convertToDate) {
			...
			FamilyMemberConverted singleEntry = FamilyMemberConverted.builder().id(fm.getId())
					.familyId(fm.getFamilyId()).name(fm.getName()).photo(fm.getPhoto()).category(fm.getCategory())
					.isRegistered(fm.isRegistered()).motherName(fm.getMotherName()).fatherName(fm.getFatherName())
					.mobileNumber(fm.getMobileNumber()).stateCode(fm.getStateCode()).idType(fm.getIdType())
					.idNumber(fm.getIdNumber()).centerId(fm.getCenterId()).centerName(fm.getCenterName())
					.relationWithOwner(fm.getRelationWithOwner()).gender(fm.getGender()).dob(df.format(formatToTime))
					.maritalStatus(fm.getMaritalStatus()).memberCode(fm.getMemberCode()).handicap(fm.getHandicap())
					.handicapType(fm.getHandicapType()).residentArea(fm.getResidentArea())
					.dateOfArrival(fm.getDateOfArrival()).dateOfLeaving(fm.getDateOfLeaving())
					.dateOfMortality(fm.getDateOfMortality()).recordForMonth(fm.getRecordForMonth()).build();
	...
	public List<AttendanceConverted> convertAttendanceUnixToDate() {
		...
		for (Attendance list : getData) {
			...
			AttendanceConverted attendance = AttendanceConverted.builder().id(list.getId()).centerId(list.getCenterId())

					.centerName(memberDetails.getCenterName()).name(memberDetails.getName())
					.centerName(list.getCenterName()).name(findMember.getName()).isRegistered(list.isRegistered())
					.childId(list.getChildId()).date(df.format(date)).attType(list.getAttType())
					.latitude(list.getLatitude()).longitude(list.getLongitude())
					.dob(commonMethodsService.dateChangeToString(memberDetails.getDob()))
					.gender(memberDetails.getGender()).dob(commonMethodsService.dateChangeToString(findMember.getDob()))
					.gender(findMember.getGender()).photo(list.getPhoto()).attendance(list.getAttendance()).build();
	...
	public AttendancePhotoDTO saveAttendancePhoto(AttendancePhotoDTO attendancePhotoDTO) {
		...
		AttendancePhoto saveAP = AttendancePhoto.builder().centerId(attendancePhotoDTO.getCenterId())
				.centerName(commonMethodsService.findCenterName(attendancePhotoDTO.getCenterId()))
				.imageName(attendancePhotoDTO.getImageName()).date(attendancePhotoDTO.getDate()).build();
	...
	public List<DashboardMaster> getDashboardMasterDetails() throws ParseException {
		...
		for (AnganwadiCenter ac : listCenters) {
			...
			DashboardMaster singleEntry = DashboardMaster.builder().centerId(ac.getId()).centerName(ac.getCenterName())
					...
					.todayAttendance(commonMethodsService.todayAttendance(ac.getId())).build();
	...
	public UpdateStudentDTO updateStudentDetails(UpdateStudentDTO updateStudentDTO) throws ParseException {
		...
		if (anganwadiChildrenRepository.findById(updateStudentDTO.getId().trim()).isPresent()) {
			...
			memberDetails.setDob(updateStudentDTO.getDob() == null ? 0L
					: commonMethodsService.dateChangeToLong(updateStudentDTO.getDob()));
			...
			ac.setIsGoingSchool(
					updateStudentDTO.getIsGoingSchool() == null ? "0" : updateStudentDTO.getIsGoingSchool());
...
// <<<<<<HEAD
					// attend.setName(updateStudentDTO.getName() == null ? "" :
					// updateStudentDTO.getName());
					// attend.setGender(updateStudentDTO.getGender() == null ? "" :
...
// branch 'master' of https://github.com/BhanuBharuwa/anganwadi-backend.git
			return UpdateStudentDTO.builder().childId(ac.getChildId() == null ? "" : ac.getChildId())
					...
					.profilePic(ac.getProfilePic() == null ? "" : ac.getProfilePic()).id(ac.getId())

					.isGoingSchool(ac.getIsGoingSchool())

					.isGoingSchool(ac.getIsGoingSchool() == null ? "" : ac.getIsGoingSchool())
////>>>> branch 'master' of https://github.com/BhanuBharuwa/anganwadi-backend.git
					.name(ac.getName() == null ? "" : ac.getName()).deleted(ac.isDeleted()).build();
	...
	public UpdateStudentDTO deleteStudentDetails(String id) {
		...
		if (anganwadiChildrenRepository.findById(id).isPresent()) {
			...
			return UpdateStudentDTO.builder().childId(findStudent.getChildId() == null ? "" : findStudent.getChildId())
					.dob(commonMethodsService.dateChangeToString(memberDetails.getDob()) == null ? ""
							: commonMethodsService.dateChangeToString(memberDetails.getDob()))
					.gender(memberDetails.getGender() == null ? "" : memberDetails.getGender())
					.profilePic(findStudent.getProfilePic() == null ? "" : findStudent.getProfilePic())
					.id(findStudent.getId()).name(findStudent.getName() == null ? "" : findStudent.getName())
	...
	public List<PartialStudentList> getStudentListByChildId(PartialStudentList partialStudentList) {
		...
		for (String childId : splitComma) {
			...
			List<AnganwadiChildren> childrenList = anganwadiChildrenRepository
					.findAllByChildIdAndRegisteredTrue(childId.trim());
			...
			for (AnganwadiChildren ac : childrenList) {
				...
				PartialStudentList singeList = PartialStudentList.builder()
						...
						.name(ac.getName() == null ? "" : ac.getName()).build();
	...
	@Override
	public List<SaveActivitiesDTO> saveActivity(AnganwadiActivitiesDTO anganwadiActivitiesDTO, String centerId)
			throws ParseException {
...
//        log.error("Attandance data " + checkAttendance);
		...
		if (ac != null) {
			...
			anganwadiActivitiesRepository.save(AnganwadiActivities.builder().centerId(centerId)
					// .centerName(centerName)
					.gaming(anganwadiActivitiesDTO.isGaming()).preEducation(anganwadiActivitiesDTO.isPreEducation())
					.cleaning(anganwadiActivitiesDTO.isCleaning()).date(timestamp).build());
		}
		...
		List<AnganwadiActivities> findAc = anganwadiActivitiesRepository.findAllByDateRange(startTime, endTime,
				centerId.trim());
		...
		for (AnganwadiActivities activities : findAc) {
			...
			addList.add(SaveActivitiesDTO.builder().id(activities.getId()).centerId(activities.getCenterId())
					.centerName(centerDetails.getCenterName())
					.childrenCount(getChildrenPresentCounts(centerId, activities.getDate()))
					.gaming(activities.isGaming()).preEducation(activities.isPreEducation())
					.cleaning(activities.isCleaning()).date(df.format(activities.getDate())).build());
		}
	...
	private long getChildrenPresentCounts(String centerId, long date) {
		...
		List<Attendance> findPresentCounts = attendanceRepository.findAllByDateAndCenterId(date, centerId,
	...
	public List<AnganwadiActivitiesDTO> getAllActivity(AnganwadiActivitiesDTO anganwadiActivitiesDTO, String centerId)
		...
		List<AnganwadiActivities> findTodayActivities = anganwadiActivitiesRepository.findAllByDateRange(startTime,
				endTime, centerId);
		...
		if (findTodayActivities.size() > 0) {
			for (AnganwadiActivities activities : findTodayActivities) {
				...
				addList.add(AnganwadiActivitiesDTO.builder().id(activities.getId()).centerId(activities.getCenterId())
						.centerName(centerDetails.getCenterName())
						.childrenCount(getChildrenPresentCounts(centerId, activities.getDate()))
						.gaming(activities.isGaming()).preEducation(activities.isPreEducation())
						.cleaning(activities.isCleaning()).date(df.format(activities.getDate()))
						.startDate(df.format(startTime)).endDate(df.format(endTime)).build());
			}
	...
	private List<FoodItemsDTO> getBreakFastItems(List<MealsType> mealsTypes) {
		...
		for (MealsType breakFast : mealsTypes) {
			...
			if (breakFast.getMealType().trim().equals("1")) {
				breakFastLists.add(FoodItemsDTO.builder().itemName(breakFast.getItemName().trim())
						.itemCode(breakFast.getItemCode().trim()).build());
			}
	...
	private List<FoodItemsDTO> getMealsList(List<MealsType> mealsTypes) {
		...
		for (MealsType meals : mealsTypes) {
			...
			if (meals.getMealType().trim().equals("2")) {
				MealsLists.add(FoodItemsDTO.builder().itemName(meals.getItemName().trim())
						.itemCode(meals.getItemCode().trim()).build());
			}
	...
	public MealTypeDTO getMealsItems() {
		...
		return MealTypeDTO.builder().breakFastLists(getBreakFastItems(mealsTypes)).mealsLists(getMealsList(mealsTypes))
				.build();
...
//        log.error("Attandance data " + checkAttendance);
		...
		for (SaveMeals mealsData : saveMeals) {
			...
			List<Meals> checkMeals = mealsRepository.findAllByMealTypeAndCenterIdAndDate(mealsData.getMealType(),
					centerId, timestamp);
			...
			if (checkItemCode.isPresent()) {
				...
				mealsRepository.save(Meals.builder().date(timestamp).itemCode(checkItemCode.get().getItemCode())
						.quantity(String.valueOf(totalQuantity)).mealType(checkItemCode.get().getMealType())
						.totalCalorie(String.valueOf(totalCalorie)).totalProtein(String.valueOf(totalProtein))
						.centerId(centerId).build());
			} else {
...
//                throw new CustomException("Selected Food Item Is Not Available, Please Contact Anganwadi To Add In List");
	...
	private List<SaveMealsItems> getMealsDetails(long date, List<Meals> findMonthlyData) {
		...
		for (Meals ml : findMonthlyData) {
			...
			if (ml.getDate() == date) {
				addMealsList.add(SaveMealsItems.builder().itemName(checkItemCode.get().getItemName())
						.itemCode(checkItemCode.get().getItemCode()).quantityUnit(checkItemCode.get().getQuantityUnit())
						.quantity(ml.getQuantity() == null ? "" : ml.getQuantity()).centerId(ml.getCenterId())
	...
	@Override
	public List<MealsResponseDTO> getMonthlyDistributedMeals(DashboardFilter dashboardFilter, String centerId)
			throws ParseException {
		DateFormat df = new SimpleDateFormat("dd-MM-yyyy");
		...
		if (findMonthlyData.size() > 0) {
			for (Meals getMeals : findMonthlyData) {
				if (uniqueDate.add(getMeals.getDate())) {
					addInList.add(MealsResponseDTO.builder().date(df.format(getMeals.getDate()))
							.childrenCount(getChildrenPresentCounts(getMeals.getCenterId(), getMeals.getDate()))
							.data(getMealsDetails(getMeals.getDate(), findMonthlyData)).build());
				}
			}
		}
	...
	public List<ChildrenDTO> getTotalChildren(String centerId) throws ParseException {
		...
		// Initially all the age group was added, but currently is was stop from backend
		// && to avoid those old records, below ,condition is added"
...
//            long dob = dob_date.getTime();
			...
			if (member.size() > 0 && getChildren.getIsGoingSchool().trim().equals("0")) {
				...
				ChildrenDTO childrenDTO = ChildrenDTO.builder().id(getChildren.getId())
						.isGoingSchool(getChildren.getIsGoingSchool() == null ? "" : getChildren.getIsGoingSchool())
						.childId(getChildren.getChildId() == null ? "" : getChildren.getChildId()).dob(dobInString)
						...
						.isRegistered(getChildren.isRegistered()).category(category)
						.name(getChildren.getName() == null ? "" : getChildren.getName()).gender(gender)
						.profilePic(getChildren.getProfilePic() == null ? "" : getChildren.getProfilePic())
						.deleted(getChildren.isDeleted()).build();
	...
	public UploadDTO uploadPic(MultipartFile file) throws IOException {
		return UploadDTO.builder().url(fileManagementService.uploadPic(file)).build();
	}
...
//        markAsAbsent(centerId);
		...
		List<Attendance> findRecords = attendanceRepository.findAllByDateAndCenterIdAndRegistered(timestamp, centerId,
		...
		for (Attendance singleRecord : findRecords) {
			...
			if (millis <= convertToMills) {
				AttendanceDTO dailyRecord = AttendanceDTO.builder()
						...
						.name(commonMethodsService.findMember(singleRecord.getChildId()).getName() == null ? ""
								: commonMethodsService.findMember(singleRecord.getChildId()).getName())
						...
						.centerName(memberDetails.getCenterName())
						.gender(commonMethodsService.findMember(singleRecord.getChildId()).getGender() == null ? ""
								: commonMethodsService.findMember(singleRecord.getChildId()).getGender())
						...
						.date(singleRecord.getDate()).build();
...
//>>>> branch 'master' of git@github.com:BhanuBharuwa/anganwadi-backend.git
    ...
    public List<AttendanceDTO> makeAndUpdateAttendance(AttendanceDTO attendanceDTO, String centerId) throws ParseException {
        ...
        for (Attendance fetchDetails : getDetails) {
        	...
        	if(!memberDetails.getDateOfMortality().equals("")) continue;
...
//>>>> branch 'master' of git@github.com:BhanuBharuwa/anganwadi-backend.git
				...
				if (uniqueFamilyIds.add(ac.getFamilyId())) {
					...
					if (checkHouseDetails.size() > 0) {
						for (Family familyDetails : checkHouseDetails) {
							...
							String headName = "", dob = "", pic = "", gender = "", houseNo = "", religion = "",
									category = "";
							...
							List<FamilyMember> findHouseholds = familyMemberRepository
									.findAllByFamilyId(ac.getFamilyId(), Sort.by(Sort.Direction.DESC, "createdDate"));
							...
							householdsHeadList householdsDTO = householdsHeadList.builder().id(familyDetails.getId())
									.headName(headName).headDob(dob).houseNo(houseNo).familyId(ac.getFamilyId())
									.religion(religion).headGender(gender)
									...
									.category(category).totalMembers(String.valueOf(countMembers)).headPic(pic).build();
	...
	public List<StockListDTO> getAvailableItems() {
		...
		for (StockList mapItems : stockLists) {
			if (uniqueStockCode.add(mapItems.getItemCode())) {
				StockListDTO assets = StockListDTO.builder().itemCode(mapItems.getItemCode())
						.quantity(mapItems.getQuantity()).unit(mapItems.getUnit()).itemName(mapItems.getItemName())
						.build();
	...
	public List<StockItemsDTO> addStocks(List<StockItemsDTO> assetsStock, String centerId) throws ParseException {
		...
		for (StockItemsDTO assetsList : assetsStock) {
			...
			AssetsStock saveStocks = AssetsStock.builder().centerId(centerId)
					.centerName(commonMethodsService.findCenterName(centerId)).qtyUnit(assetsList.getUnit())
					.itemCode(assetsList.getItemCode()).itemName(assetsList.getItemName()).qty(assetsList.getQuantity())
					.date(timestamp).month(currentMonth).build();

			assetsStockRepository.save(saveStocks);

			StockItemsDTO addSingleItem = StockItemsDTO.builder().centerId(centerId)
					.centerName(commonMethodsService.findCenterName(centerId)).unit(assetsList.getUnit())
					.itemCode(assetsList.getItemCode()).itemName(assetsList.getItemName())
					.quantity(assetsList.getQuantity()).date(formatToString).build();
...
//    }
	...
	private List<DistributionArrayList> getItemArray(String familyId, String selectedMonth) {
		...
		for (StockDistribution items : findItems) {
			...
			if (uniqueItem.add(items.getItemCode())) {
				...
				DistributionArrayList singeList = DistributionArrayList.builder().itemName(items.getItemName())
						.itemCode(items.getItemCode()).unit(items.getUnit())
						.quantity(getDistributionQty(items.getFamilyId(), items.getItemCode(), selectedMonth)).build();
	...
	public List<DistributionOutputList> getDistributionList(String centerId, String selectedMonth)
		...
		List<StockDistribution> findFamily = stockDistributionRepository.findAllByCenterIdAndDateRange(centerId,
				startDataRange, endDataRange);
...
//        log.error("using center Name: "+findFamily1);
		...
		for (StockDistribution sd : findFamily) {
			...
			List<FamilyMember> findHeadDetails = familyMemberRepository.findAllByFamilyId(sd.getFamilyId(),
					Sort.by(Sort.Direction.DESC, "createdDate"));
			...
			if (uniqueFamily.add(sd.getFamilyId())) {
				...
				DistributionOutputList singleEntry = DistributionOutputList.builder().name(name).familyId(familyId)
						.profilePic(profilePic).houseNo(houseNo).date(df.format(date))
						.arrayLists(getItemArray(sd.getFamilyId(), selectedMonth)).build();
				addInList.add(singleEntry);
			}
		}
		return addInList;
...
//    }
	...
	public List<WeightTrackingDTO> getChildrenWeightData(DashboardFilter dashboardFilter) throws ParseException {
		...
		List<WeightTracking> findChildren = weightTrackingRepository.findAllByMonthCriteria(startTime, endTime,
				Sort.by(Sort.Direction.ASC, "createdDate"), dashboardFilter.getCenterId().trim());
...
//    }
	...
	public List<AnganwadiChildrenList> getAnganwadiChildrenDetails(DashboardFilter dashboardFilter)
		...
		List<AnganwadiChildren> childrenList = anganwadiChildrenRepository.findAllByCreatedDateAndSearch(startTime,
				endTime, searchKeyword.trim(), dashboardFilter.getCenterId().trim());
			...
			if (dataList.isRegistered() && dataList.getIsGoingSchool().equals("0")) {
				if (uniqueStudent.add(dataList.getChildId())) {
					...
					AnganwadiChildrenList addSingle = AnganwadiChildrenList.builder()
							...
							.dob(commonMethodsService.dateChangeToString(memberDetails.getDob()))
							.category(memberDetails.getCategory() == null ? "" : memberDetails.getCategory())
							.religion(findReligion.getReligion() == null ? "" : findReligion.getReligion()).build();
					addInList.add(addSingle);
				}
			}
		}
	...
	public List<AttendanceDTO> makeAttendance(AttendanceDTO attendanceDTO) throws ParseException {
		...
		for (AnganwadiChildren getId : findChildren) {
			...
			Attendance saveAttendance = Attendance.builder().childId(getId.getChildId())
					...
					.latitude(attendanceDTO.getLatitude()).longitude(attendanceDTO.getLongitude())
					.photo(getId.getProfilePic())
					// .gender(memberDetails.getGender())
					.date(timestamp).attendance(attendance).build();
			attendanceRepository.save(saveAttendance);
			...
			try {
				if (checkChild.size() > 0) {
					for (AnganwadiChildren children : checkChild) {
						List<Attendance> updateAttendance = attendanceRepository
								.findAllByChildId(children.getChildId());
		...
		List<Attendance> getDetails = attendanceRepository.findAllByDate(currentDate,
				Sort.by(Sort.Direction.DESC, "createdDate"));

		for (Attendance fetchDetails : getDetails) {
			FamilyMember memberDetails = familyMemberRepository.findById(fetchDetails.getChildId()).get();

			AttendanceDTO singleEntry = AttendanceDTO.builder().childId(fetchDetails.getChildId())
					.dob(commonMethodsService.dateChangeToString(memberDetails.getDob())).name(memberDetails.getName())
					// .dob(fetchDetails.getDob())
					// name(commonMethodsService.findMember(fetchDetails.getChildId()).getName())
////>>>> branch 'master' of git@github.com:BhanuBharuwa/anganwadi-backend.git
					.latitude(attendanceDTO.getLatitude()).longitude(attendanceDTO.getLongitude())
					.photo(fetchDetails.getPhoto()).gender(memberDetails.getGender()).date(currentDate)
					.attendance(fetchDetails.getAttendance()).build();
